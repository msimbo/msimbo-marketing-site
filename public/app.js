...
(function () {
  ...
  const INFO_SESSION_DATES = [
    { label: "Thu, May 21", time: "6:00 PM", value: "2026-05-21T18:00:00" },
  ];
  ...
  // ──────────────────────────────────────────────
  // COHORT 2 WAITLIST SUBMISSION
  // ──────────────────────────────────────────────
  function submitWaitlist(formEl) {
    var btn = document.getElementById("waitSubmitBtn");
    var formErrorSummary = document.getElementById("waitFormError");
    ...
    var payload = {
      signupType: "waitlist",
      firstName: document.getElementById("waitFirstName").value.trim(),
      lastName: document.getElementById("waitLastName").value.trim(),
      email: document.getElementById("waitEmail").value.trim(),
      phone: document.getElementById("waitPhone").value.trim(),
  ...
  function validateWaitlistForm() {
    var fields = [
      { id: "waitFirstName", err: "waitFirstNameError", label: "First name" },
      { id: "waitLastName", err: "waitLastNameError", label: "Last name" },
      { id: "waitEmail", err: "waitEmailError", label: "Email" },
      { id: "waitPhone", err: "waitPhoneError", label: "Phone" },
    ];
    var ok = true;
    ...
    var emailEl = document.getElementById("waitEmail");
    if (emailEl && emailEl.value.trim() && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(emailEl.value.trim())) {
      var emailErr = document.getElementById("waitEmailError");
      if (emailErr) emailErr.textContent = "Enter a valid email address";
      ok = false;
    }
    var phoneEl = document.getElementById("waitPhone");
    if (phoneEl && phoneEl.value.trim() && phoneEl.value.replace(/\D/g, "").length < 10) {
      var phoneErr = document.getElementById("waitPhoneError");
      if (phoneErr) phoneErr.textContent = "Enter a 10-digit phone number";
      ok = false;
    }
    return ok;
  }
