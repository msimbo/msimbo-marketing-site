...
(function () {
  ...
  const INFO_SESSION_DATES = [
    { label: "Thu, Jun 18", time: "6:00 PM", value: "2026-06-18T18:00:00" },
    { label: "Thu, Jun 25", time: "6:00 PM", value: "2026-06-25T18:00:00" },
  ];
  ...
  function renderDatePicker() {
    ...
    INFO_SESSION_DATES.forEach(function (session, i) {
      ...
      card.innerHTML =
        '<span class="date-option__day">' +
        session.label +
        "</span>" +
        '<span class="date-option__time">' +
        session.time +
        "</span>";
  ...
  function fireConversionEvents(signupType, payload) {
    ...
    var metaEvent =
      signupType === "application" ? "CompleteRegistration" : "Lead";
  ...
  function initApplicationWizard(formEl) {
    ...
    function validateStep(step) {
      ...
      if (step === 1) {
        ["appFirstName", "appLastName", "appEmail", "appPhone"].forEach(
          function (id) {
        ...
        if (firstName.length < 1) {
          setFieldError("appFirstNameError", "First name is required");
          valid = false;
        }
        if (lastName.length < 1) {
          setFieldError("appLastNameError", "Last name is required");
          valid = false;
        }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
          setFieldError("appEmailError", "Enter a valid email address");
          valid = false;
        }
        var phoneDigits = phone.replace(/\D/g, "");
        if (phoneDigits.length < 10) {
          setFieldError("appPhoneError", "Enter a valid phone number");
          valid = false;
        }
      }
      ...
      if (step === 2) {
        ...
        if (!dob) {
          setFieldError("appDobError", "Date of birth is required");
          valid = false;
        } else {
          var age = calculateAge(dob);
          if (age < 18) {
            setFieldError("appDobError", "You must be 18 or older to apply.");
            valid = false;
          } else if (age > 120) {
            setFieldError("appDobError", "Please enter a valid date of birth.");
            valid = false;
          }
        }

        if (!/^[0-9]{5}$/.test(zip)) {
          setFieldError("appZipError", "Enter a 5-digit zip code");
          valid = false;
        }
      ...
      if (step === 3) {
        ...
        var neighborhood = document
          .getElementById("appNeighborhood")
          .value.trim();
        ...
        if (neighborhood.length < 2) {
          setFieldError("appNeighborhoodError", "Neighborhood is required");
          valid = false;
        }
        if (language.length < 2) {
          setFieldError("appLanguageError", "Primary language is required");
          valid = false;
        }
        if (!employment) {
          setFieldError(
        ...
        if (!referral) {
          setFieldError(
            "appReferralError",
            "Please select how you heard about RTS",
          );
          valid = false;
        }
      }
      ...
      if (step === 4) {
        ...
        var motivationNow = document
          .getElementById("appMotivationNow")
          .value.trim();
        var motivationGoal = document
          .getElementById("appMotivationGoal")
          .value.trim();

        if (motivationNow.length < 50) {
          setFieldError(
            "appMotivationNowError",
            "Please share at least 50 characters (" +
              motivationNow.length +
              " so far).",
          );
          valid = false;
        }
        if (motivationGoal.length < 50) {
          setFieldError(
            "appMotivationGoalError",
            "Please share at least 50 characters (" +
              motivationGoal.length +
              " so far).",
          );
          valid = false;
        }
        ...
        .then(function (result) {
          if (!result.ok) {
            ...
            if (result.status === 409) {
              ...
              formErrorSummary.textContent =
                "Something went wrong. Please try again, or contact program-rts@ulem.org.";
            }
            return;
          }
    ...
    headline.addEventListener("click", function () {
      ...
      navigator.clipboard
        ...
        .then(function () {
          ...
          setTimeout(function () {
            headline.classList.remove("link-copied");
          }, 1500);
        })
        .catch(function () {
